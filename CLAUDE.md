...
## 16. 색상 팔레트 · 폰트 리브랜딩 — 1단계만 완료, 2단계는 화면별로 순차 진행 (2026-09-08)
...
- ✅ **2단계 1차 완료(2026-09-08)**: 템플릿 전체에서 가장 눈에 띄던 "쨍한 구 브랜드
  ...
  훨씬 빠르다는 걸 확인함.
- ⬜ **2단계 남은 부분**: 템플릿 65개 중 60개에 색상이 하드코딩돼 있던 것 중(원래
  1,734곳), 이번에 처리한 파랑 계열 104곳을 뺀 **나머지(회색/보더 계열 —
  `#e5e7eb`/`#f3f4f6`/`#d1d5db`/`#f9fafb`/`#374151`/`#1f2937` 등, 그리고 보라·청록
  등 개별 강조색)는 아직 손 안 댔다.** 판정 의미 색상(`#dc2626`/`#16a34a`/`#d97706`
  계열)과 "info" 의미 연한 파랑(`#eff6ff`/`#bfdbfe`/`#1e3a8a`)은 **의도적으로 건드리지
  않는 대상**이니 이것들을 "아직 안 바뀐 버그"로 착각하지 말 것. 회색 계열은 사용자가
  지적한 "쨍한 색" 문제의 핵심이 아니었어서(이미 채도가 낮음) 우선순위가 낮다 —
  다음에 진행하려면 먼저 어떤 값들을 어느 새 톤에 맞출지부터 정해야 한다.
  **"디자인이 반만 바뀐 것 같다"는 얘기가 나오면 버그가 아니라 이 부분이 아직
  안 끝난 것**임을 먼저 확인할 것.
