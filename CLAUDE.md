...
- ⬜ **2단계 미착수**: 템플릿 65개 중 60개에 색상이 **하드코딩**돼 있고(1,734곳),
  대부분 `var(--primary)` 등을 안 쓰고 직접 hex를 찍어놔서 **1단계만으로는 이 부분이
  전혀 안 바뀐다.** 화면 전체를 진짜로 눈피로 낮춘 톤으로 통일하려면, 지난 7개 화면
  디자인 감사 때처럼 화면별로 순차적으로(designer + quality-watcher 검증) 하드코딩된
  색을 정리해야 한다 — 이건 사용자와 합의된 대로 다음 세션들에서 진행할 큰 작업이다.
  **"디자인이 반만 바뀐 것 같다"는 얘기가 나오면 버그가 아니라 이 2단계가 아직
  안 끝난 것**임을 먼저 확인할 것.
