...
## 14. 승인목록/승인이력 삭제버튼은 admin 전용이 맞다 (2026-09-07 확정, 되돌리지 말 것)

`templates/_admin_delete.html`의 `admin_delete_bar` 매크로는 `show` 인자로 노출 조건을
바꿀 수 있다(기본값은 `is_admin`). 이 삭제 버튼을 쓰는 4개 화면 중:

- **승인목록(`approve_list.html`)·승인이력(`approval_history.html`)** → `show=is_admin`
  (admin 계정만)
- **출력대기(`output_list.html`)·출력이력(`output_history.html`)** → `show=('history_delete'
  in user_perms)` (그 권한을 가진 계정이면 누구나)

**둘이 다르게 동작하는 건 실수가 아니라 확정된 설계다.** 승인 관련 삭제는 더 민감해서
admin으로 한정하고, 출력 관련 삭제는 `history_delete` 권한 세분화 원칙(5절)을 그대로
적용한다. 세션이 바뀔 때마다 "이게 버그 아니냐"고 반대 방향으로 계속 고치는 일이
실제로 있었으니(한 세션이 권한기반→admin전용으로, 다른 세션이 admin전용→권한기반으로
서로 되돌린 이력이 있음), **4개 화면 전부 동일하게 맞추려 하지 말 것.** 각 파일의
`admin_delete_bar(...)` 호출 바로 위에 이 의도를 설명하는 주석이 있으니 먼저 확인해라.

**일반적 교훈**: 같은 로컬 저장소(`C:\Users\Jaiden\Desktop\iqc-app`)를 여러 Claude
세션이 시차를 두고 건드릴 수 있다. 커밋 메시지에 "버그 수정"이라고 적혀 있어도, 그게
실제로는 다른 세션이 사용자와 논의해서 내린 의도적 설계일 수 있다 — 코드를 "고치기"
전에 `git log -p`로 관련 커밋 메시지를 먼저 읽고, 의도가 불분명하면 되돌리지 말고
사용자에게 확인해라.

## 15. 자재 메뉴 재구성 (2026-09-07) — "자재 관리"는 그대로, "일괄등록" 이름만 이동

자재 관련 메뉴가 3개에서 2개로 줄었다. **자재번호별 이름이 여러 번 뒤바뀌며 혼선이
있었던 결정이니, 다시 고칠 일이 있으면 아래 표부터 확인할 것.**

| 화면(route) | 이전 메뉴 이름 | 지금 |
|---|---|---|
| `spec_list` (`/spec`) | 자재 관리 | **그대로 "자재 관리"** — 이름도 기능도 안 바꿈 |
| `spec_detail` (`/spec/<material_no>`, 자재관리에서 자재 클릭 시 진입) | (별도 메뉴 없음) | 이 화면이 곧 "자재 개별등록" 개념이다 — 기존에 등록된 자재 하나를 찾아 수정하는 화면. **별도 메뉴 링크는 추가하지 않았다**(자재관리 목록에서 클릭해서 들어가는 걸로 충분) |
...
**다시 헷갈리지 않도록**: "자재 개별등록"은 **메뉴 항목이 아니다.** `spec_detail`
화면 자체(자재관리 → 자재 클릭)를 가리키는 개념일 뿐이니, 나중에 "자재 개별등록
메뉴가 안 보인다"는 얘기가 나와도 새로 만들 필요 없다 — 원래 없는 게 맞다.
