...
## 26. 프로덕션 배포 후 실제로 터진 함정 3종 (2026-09-26, 하우징 프로젝트 배포 직후)

하우징 성적서 프로젝트(신규 컬럼·신규 테이블·1회성 데이터 마이그레이션 스크립트가
같이 딸린 대규모 배포)를 배포한 직후 실제로 겪은 사고 3건. **다음에 이런 배포(신규
DB컬럼 추가 + 그걸 채우는 1회성 스크립트)를 할 때 반드시 먼저 참고할 것.**

### 26-1. gunicorn 멀티워커가 같은 `ALTER TABLE ADD COLUMN`을 동시에 실행해서 부팅 자체가 실패함

Render는 gunicorn 워커를 여러 개(이 프로젝트는 2개) 띄우는데, 각 워커는 **독립
프로세스**라 앱 모듈이 import될 때(`db.init_db()`가 모듈 레벨에서 호출됨, §"DB
초기화를 모듈 레벨로 이동" 참고) 워커마다 각자 `init_db()`를 실행한다. **그날 처음
추가되는 컬럼**에 대해선 두 워커가 "컬럼 없음"(`existing_cols` 체크)을 프로덕션 DB에
아직 그 컬럼이 없는 상태에서 **거의 동시에** 통과해버리고, 둘 다 `ALTER TABLE ADD
COLUMN`을 시도한다 — SQLite의 파일 잠금 때문에 한쪽이 블록됐다가 먼저 커밋한 워커
뒤에 실행되면서 `sqlite3.OperationalError: duplicate column name`으로 실패 → 그
워커가 죽고 gunicorn이 "Worker failed to boot"로 마스터까지 셧다운 → **사이트 전체가
다운**됐다(실제 프로덕션 장애, 로그로 확인).

SQLite는 `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` 문법이 없어서, `existing_cols`
사전 체크만으론 이 레이스를 원천 차단할 수 없다 — **체크와 실행 사이의 시간차** 자체가
문제이기 때문이다. `database.py`에 `_safe_add_column(cur, alter_sql)` 헬퍼를 만들어서
해결했다: `ALTER TABLE ADD COLUMN`을 실행하고, `sqlite3.OperationalError`가 나면
메시지에 `"duplicate column name"`이 있을 때만 무시(그 외 에러는 그대로 raise). 기존
`existing_cols` 체크는 그대로 두고(불필요한 재시도를 줄여주는 최적화로는 여전히 유효),
실행 시점의 경합만 이 헬퍼가 흡수한다.
...
**앞으로 `database.py`에 새 `ALTER TABLE ... ADD COLUMN`을 추가할 땐 항상
`_safe_add_column()`을 거쳐서 실행할 것.** 이 프로젝트엔 이미 수십 개의 ALTER TABLE이
있는데(4-1절 이전부터 계속 추가돼 온 것들) 여태 이 문제를 한 번도 안 겪은 건 프로덕션이
정상 작동해서가 아니라 순전히 **워커 부팅 타이밍이 우연히 안 겹쳤을 뿐**이다 — 구조적
위험은 그 모든 ALTER TABLE에 똑같이 있다. 기존 것들을 전부 지금 당장 고칠 필요는
없지만(과설계 방지), 다음에 그중 하나를 손댈 일이 생기면 그 김에 이 헬퍼로 바꿔둘 것.

### 26-2. 프로덕션에서 돌리는 1회성 마이그레이션 스크립트가 `sqlite3.connect('iqc.db')`(상대경로)를 쓰면 엉뚱한(빈) DB를 본다

`migrate_housing*.py` 같은 1회성 스크립트들은 원래 로컬 개발 PC에서만 실행할 걸 가정하고
`sqlite3.connect('iqc.db')`로 짜여 있었다. 로컬에선 스크립트 실행 위치가 곧 `iqc.db`가
있는 위치라 문제가 없었는데, **프로덕션(Render)은 실제 DB가 `DATA_DIR`(영구디스크
`/var/data`)에 있고 앱 코드는 `/app`에 있다** — Render Shell은 기본적으로 `/app`에서
열리므로, 이 상대경로가 `/app/iqc.db`(존재하지 않는 빈 경로)를 가리켜서
`sqlite3.OperationalError: no such table: specs`로 실패했다.

고친 방법: 스크립트 맨 위에 `import database as db`를 추가하고
`sqlite3.connect(db.DB_PATH)`로 바꿨다 — `database.py` 자신이 쓰는 것과 항상 같은
경로를 보장한다. **앞으로 프로덕션에서 한 번이라도 돌릴 가능성이 있는 1회성 DB
스크립트는 절대 `sqlite3.connect('iqc.db')`처럼 하드코딩하지 말고, 반드시
`db.DB_PATH`를 재사용할 것.**

### 26-3. 로컬 개발DB의 데이터 내용이 프로덕션과 다를 수 있다 — 스키마 검증은 로컬로 충분해도, 데이터 의존적 로직은 프로덕션 실측 없이 확정 짓지 말 것

하우징 2단계에서 "M항목(X-선검사)을 어느 차수로 분류할지" 결정할 때, 로컬 개발DB를
기준으로 판단했다(로컬엔 M=X-선검사, N=외관검사가 별도 항목으로 둘 다 존재했음). 그런데
실제 프로덕션 DB를 마이그레이션 스크립트로 조회해보니 완전히 달랐다 — `602013P015`는
`N`=외관검사만 있고 `M` 자체가 없고, `604013P000`/`608013P011`은 `M`=외관검사만 있고
`N`이 없었다. **프로덕션엔 X-선검사라는 별도 항목이 애초에 존재하지 않았다** — 로컬DB의
"M=X-선검사"는 실사용 데이터가 아니라 로컬에만 있던 테스트 데이터였던 것으로 보인다
(정확한 경위는 추적 안 함). "M은 아직 미사용이니 더미(비활성) 처리"라는 결정을 그대로
프로덕션에 적용했더니, 실제로는 27KV/38KV 두 자재의 **진짜 외관검사 항목을 지워버리는
사고**가 됐다(2차 화면에서 안 보이게 됨) — 마이그레이션 재실행으로 정정.

**교훈**: 이 프로젝트는 "로컬 PC는 코딩 전용, Render가 정식 운영 서버"(§project 메모리
참고)라는 원칙이 이미 있는데, 이번 건은 그 원칙이 **스키마 검증(테이블/컬럼 존재 여부,
멱등성 등)을 넘어 데이터 내용(어떤 자재가 실제로 어떤 항목을 갖고 있는지)에도 그대로
적용된다는 걸 보여준 사례**다. 자재별 실제 항목 구성처럼 "그 자재에 실제로 뭐가 들어있는지"에
의존하는 로직(이번처럼 item_name 문자를 하드코딩해서 의미를 부여하는 마이그레이션 등)은,
로컬DB로 사전 테스트해서 통과했더라도 **프로덕션에서 실측(Shell로 직접 조회) 후 최종
확정할 것** — 스키마가 같다고 데이터도 같다고 가정하지 말 것.
