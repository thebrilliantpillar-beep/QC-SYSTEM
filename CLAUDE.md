...
### 8-2-1. AQL 표기는 두 줄 (2026-08-23, 2026-09-08 표기 간소화·용어 순화)
...
불량 1개까지 합격
```
(무결점 판정 항목이면 `[0.65]` / `무결점` 처럼 둘째 줄이 "무결점"만 남는다)
...
처리한다 — 즉 표기와 실제 판정이 정확히 일치한다. 둘째 줄은 그 Ac값을 사람이 바로
알아볼 수 있게 풀어쓴 것뿐이다.
...
첫 줄은 `[숫자]`처럼 대괄호로 감싸고, 둘째 줄에 "샘플 N개/"는 안 넣는다 — 샘플
수량은 바로 옆 D열("샘플 수량")에 이미 따로 나오는데 여기서 또 반복하면 중복이다.
