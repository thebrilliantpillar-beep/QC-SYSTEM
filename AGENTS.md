...
샤든그룹(배전용 전력기기 제조업체 — 리클로저 등)의 **입고 자재 검사 성적서**를 수기 작성에서
...
**부적합 통보서(NCR) 확인은 최종결정권자 게이트를 쓰지 않는다** — 2026-09-10 변경.
`ncr_confirm` 권한만 있으면 되고 서명은 선택 사항이다 (CLAUDE.md 8-2절 참고).
서명 파일명은 `static/signatures/ncr{id}.png`.

### 8-2-1. AQL 표기 (구버전 — 2026-09-08에 3줄 형식으로 변경됨, CLAUDE.md 8-2-1절 참고)

성적서 AQL 칸(C열)은 현재 **3줄** 형식이다(`[4]` / `불량 1개` / `까지 합격`).
이 아래 내용은 구버전(2줄)이며 현재 코드와 다르다.
...
- "~해줘"는 편하게 하는 말버릇이고, **실제 코드 작성은 "코드 짜줘" 등 명시적 확인 후에만**
  진행하는 걸 선호했다(Codex에선 이 구분이 덜 중요할 수 있지만, 특히 범위가 크거나
