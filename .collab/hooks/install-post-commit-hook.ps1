...
foreach ($hookName in @('post-commit','pre-commit')) {
    ...
    if (Test-Path -LiteralPath $target) {
        ...
        if ($existing -match 'QMS collaboration (audit hook|pre-commit boundary)') { $content = [System.IO.File]::ReadAllText($source); [System.IO.File]::WriteAllText($target, $content, (New-Object System.Text.UTF8Encoding($false))); Write-Host "QMS $hookName hook을 최신 원본으로 갱신했습니다: $target"; continue }
