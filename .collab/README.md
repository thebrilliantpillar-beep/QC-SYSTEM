...
### .collab과 git log, 뭘 봐야 하나 (2026-09-24)

**"왜 이렇게 만들었는지"는 `.collab`(TASK #18~#42가 2026-08-21~09-21 구간을 CLAUDE.md
기반으로 사후정리한 것), "실제로 뭐가 바뀌었는지(diff)"는 항상 `git log`/`git show`/
`git blame`이 원본이다.** 둘을 억지로 1:1 연결하지 않는다 — 특히 2026-08-22·08-23에
CLAUDE.md가 적어둔 설계는 이 저장소의 실제 커밋 날짜론 0건이고 2026-08-24 하루의
대형 커밋 묶음(`c83c803` 등 20개)에 몰아서 반영돼 있다(claude.ai 채팅에서 먼저 설계된
뒤 나중에 커밋된 것으로 추정). 그런 구간까지 TASK 하나를 특정 커밋 하나에 억지로
연결하면 그 커밋에 같이 들어간 무관한 변경들 때문에 오히려 오해를 준다. 코드가 실제로
어떻게 바뀌었는지 정확히 보고 싶으면 그냥 `git log -p <경로>`/`git blame`을 쓸 것 —
.collab에 diff를 복제하지 않는다.
